...
def display_and_save_uploaded_image():
    ...
    # Placeholder for segmented image display
    segmented_image_placeholder = st.empty()
    ...
    if st.session_state.segmentation_text_clicked:
        ...
        segmented_image_placeholder.image(st.session_state.segmented_image, caption="Segmented Image", use_column_width=True)

        # Display the extracted text
        text_placeholder.write(f"Extracted Text: {st.session_state.extracted_text}")
        ...
        # st.write(f"Question saved to: {question_file_path}")
        ...
        # st.write(f"Reference answer saved to: {reference_answer_file_path}")

        # Add a button to grade the answer
        if st.button("Grade"):
            # Get question, reference answer, and candidate answer from files
            question = read_text_from_file(question_file_path)
            reference_answer = read_text_from_file(reference_answer_file_path)
